...
public:
    ...
    virtual void rotate(float angleDeg) = 0;      // obrót (w stopniach)
    virtual void scale(float sx, float sy) = 0;    // skalowanie
};

// Wykorzystanie wirtualnego wielodziedziczenia
...
public:
    ...
    void drawLineIncremental(sf::RenderTarget& target,
        ...
        // Odcinki strome: zamieniamy rolę x/y
    ...
    // Elipsa – parametryzacja + 4-krotna symetria (instrukcja 3)
    void drawEllipse(sf::RenderTarget& target,
        ...
        for (unsigned int i = 0; i <= steps; ++i) {
            float alpha = (pi / 2.f) * (float)i / (float)steps;  // 0..pi/2
...
public:
    Engine()
        ...
        m_window.setFramerateLimit(60);  // regulacja FPS (timer)
